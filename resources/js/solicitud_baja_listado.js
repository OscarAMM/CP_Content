...
$(document).ready(function () {
	...
	$("#tablaBienesCCT").jqGrid({
		...
		postData: {
			"cp_hsh4_tk": function () { return ($('#token').val() != "") ? $('#token').val() : "0"; }, 'sinNoAutorizados': '1'
		},
		datatype: 'json',
		mtype: 'POST',
		colNames: ['idBienes', 'Cant.', 'No. Inventarios', 'Descripcion', 'Marca', 'Modelo', 'Serie', 'Responsable', 'Fecha Alta', 'Salida', 'inventario'//, 'Detalle'
		],
		colModel: [
			{ name: 'nIdBienes', index: 'nIdBienes', width: '50px', search: false, resizable: false, align: "center", hidden: true, sortable: false },
			{ name: 'nBienes', index: 'nBienes', width: '45px', search: false, resizable: false, align: "left", sortable: true },
			{ name: 'nInventarios', index: 'nInventarios', width: '200px', search: false, resizable: true, align: "center", sortable: true },
			{ name: 'DescripcionBien', index: 'DescripcionBien', width: '300px', search: true, resizable: false, align: "left", sortable: true },
			...
			{ name: 'SerieBien', index: 'SerieBien', width: '90px', search: true, resizable: false, align: "center", sortable: true },
			...
			//{name:'action1', index:'action1', width:'60px', align:"center", search:false, resizable:false, sortable:false}
		],
		gridComplete: function () {
			...
			for (var i = 0; i < ids.length; i++) {
				...
				gridx.jqGrid('setRowData', rowId, { action1: VistaP });
			}
		},
		shrinkToFit: false,
		height: '300px',
...
function validarIncluirBienes(nombreForm) {
	...
	if (idSolicitud == '') { mensaje('Aviso sistema', 'Ocurrió un error en el formulario. Por favor actualice la página e intente nuevamente', 'ERROR'); return false; }
